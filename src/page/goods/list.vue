        ...
        <el-table-column label="审核状态" width="120" align="center" v-if="searchForm.tab != 'delete'">
          <template #default="{ row }">
            <div class="flex flex-col" v-if="row.ischeck == 0">
              <el-button type="success" size="small" @click="handleIsCheck(1,row.id)" plain>审核通过</el-button>
              <el-button class="mt-2 !ml-0" type="danger" size="small"  @click="handleIsCheck(2,row.id)" plain>审核拒绝</el-button>
            </div>
...
const {
  ...
  updateIsCheckStatus:changeGoodsStatus
})
