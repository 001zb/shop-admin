<template>
    <el-aside class="image-aside" v-loading:loading>
        <div class="top">
            <AsideList :active="activeId == item.id " @delete="handleDelete(item.id)" @edit="handleEdit(item)" v-for="(item,index) in list" :key="index" active @click="handleChangeActiveId(item.id)">{{ item.name }}</AsideList>
        </div>
        <div class="bottom">
    ...
    <formDrawer :title="drawerTitle" ref="formDrawerRef" @submit="handleSubmit" confirmText="添加">
...
const handleSubmit = ()=>{
    formRef.value.validate((valid)=>{
        ...
        fun.then((res) => {
            ...
            getData(editId.value?currentPage: 1)
